class ChatbotApp {
    ...
    bindEvents() {
        ...
        this.elements.speakBtn.addEventListener('click', () => this.speakLastMessage());
    ...
    async loadModels() {
        try {
            ...
            models.forEach(model => {
                ...
                option.textContent = `${model.displayName}${model.supportsImages ? ' 🖼️' : ''}`;
    ...
    updateModelIndicator() {
        ...
        if (this.selectedImages.length > 0 && this.originalModel && this.currentModel.name !== this.originalModel) {
    ...
    autoSwitchModel() {
        if (this.selectedImages.length > 0) {
            // Need a vision model
            const visionModels = window.groqAPI.getModels().filter(m => m.supportsImages);
    ...
    addMessageToUI(role, content, model = null, images = null) {
        ...
        avatar.innerHTML = role === 'user' ? 
            '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C13.1 2 14 2.9 14 4C14 5.1 13.1 6 12 6C10.9 6 10 5.1 10 4C10 2.9 10.9 2 12 2ZM21 9V7L15 7V9L21 9ZM3 9V7L9 7V9L3 9ZM12 7C13.1 7 14 7.9 14 9V22H16V23H8V22H10V9C10 7.9 10.9 7 12 7Z"/></svg>' :
        ...
        if (role === 'assistant' && model) {
            const modelInfo = window.groqAPI.getModels().find(m => m.name === model);
            if (modelInfo) {
                const modelIndicator = document.createElement('div');
                modelIndicator.className = 'model-indicator';
                modelIndicator.innerHTML = `
                    <span>${modelInfo.displayName}</span>
                    ${modelInfo.supportsImages ? '<span class="image-support-badge">Vision</span>' : ''}
                `;
    ...
    async sendMessage() {
        ...
        if ((!message && this.selectedImages.length === 0) || this.isProcessing) return;
        ...
        try {
            ...
            this.addMessageToUI('user', message || "[Images uploaded]", null, this.selectedImages.length > 0 ? this.selectedImages : null);
            
            // Save user message
            await window.mongoClient.saveMessage('user', message || "[Images uploaded]", this.currentModel?.name);
            ...
            await window.mongoClient.saveMessage('assistant', response.content, response.model);
            
            // Update local chat history
            this.chatHistory.push(
                { role: 'user', content: message || "[Images uploaded]", model: this.currentModel?.name },
        ...
        try {
            ...
            this.elements.chatMessages.innerHTML = `
                <div class="welcome-message">
                    ...
                    <p>Start a conversation with advanced AI models. Your chat history is automatically saved.</p>
                </div>
            `;
